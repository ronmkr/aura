...
impl TokenBucket {
    pub fn new(rate_per_sec: u64, refill_interval_ms: u64) -> Self {
        ...
        tokio::spawn(async move {
            let mut tick = interval(Duration::from_millis(refill_interval_ms));
            loop {
                tick.tick().await;
                ...
                let refill_amount = (rate as f64 / ticks_per_sec) as usize;
                let current = available_clone.available_permits();
                if current < cap as usize {
                    let to_add = std::cmp::min(refill_amount, cap as usize - current);
                    if to_add > 0 {
