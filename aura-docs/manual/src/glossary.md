...
### Fast Resume
A mechanism that allows Aura to quickly verify existing data on disk and resume a download from where it left off, rather than re-downloading from scratch (Decision 0068).
...
### History Log
A persistent, append-only record of every task that has completed, failed, or been removed from the active queue (Decision 0062).
...
A suite of cryptographic protocols used to obfuscate BitTorrent traffic, preventing Deep Packet Inspection (DPI) and throttling by ISPs (Decision 0066).
...
### ProtocolDetector
A centralized component that automatically infers the download protocol (HTTP, FTP, BitTorrent, Metalink) from a URI or local path, enabling seamless "no-type" ingestion (Decision 0065).

### Protocol Worker
A lightweight actor in Aura that handles a specific network protocol (e.g., the `HttpWorker`).

### Selective Downloading
The ability to choose specific files within a multi-file BitTorrent swarm or Metalink package to download, saving time and disk space (Decision 0065).
...
### Tenant
An isolated environment within the Aura Daemon (Decision 0032) that provides dedicated bandwidth limits, task quotas, and directory roots for multi-user shared hosting.

### ViewRouter
The architectural pattern used in the TUI to manage a navigable stack of interactive screens (Dashboard, Mission Control, File Selector) using a stateful enum (Decision 0065).
