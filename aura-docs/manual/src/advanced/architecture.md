...
- **Watch Folder Auto-Ingestion**: Filesystem watch loop utilizing `notify` with an active 500ms file-size stabilization loop to debounce writes and ingest torrents/metalinks (ADR-0069).
- **RSS Subscription Poller**: Background feed polling loop inside the daemon to automatically fetch, filter, and ingest matching feed downloads (ADR-0070).
- **System Service Control**: Native system manager service configuration (systemd, launchd, Windows SCM) supporting daemon auto-start on boot (ADR-0071).
