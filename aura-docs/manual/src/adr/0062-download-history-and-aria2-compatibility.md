...
## Edge Cases
1. **Concurrent Writes**: Multiple tasks completing simultaneously must not corrupt the history log. Use file-level advisory locking (consistent with Decision-0006) when appending, or use an actor-based history writer that serializes all writes.
