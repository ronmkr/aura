...
## Decision
We will modernize the `aura-tui` and update the `aura-core` engine to support a highly interactive, stateful architecture.
...
- **Cons**:
  - Introduces significant complexity to the `PiecePicker` and `Storage` engines to correctly map and skip unselected pieces while handling piece overlaps.
