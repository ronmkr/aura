...
use super::{Orchestrator, SubTaskEvent, Event};
...
impl Orchestrator {
    ...
    pub(crate) async fn handle_subtask_matured(&mut self, meta_id: TaskId, sub_id: TaskId, metadata: crate::worker::Metadata) -> Result<()> {
    ...
    pub(crate) async fn handle_range_finished(&mut self, meta_id: TaskId, sub_id: TaskId, range: crate::task::Range) -> Result<()> {
        if let Some(meta_task) = self.tasks.get_mut(&meta_id) {
