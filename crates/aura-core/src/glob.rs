    ...
    while expanded.iter().any(|u| range_re.is_match(u)) {
        ...
        for base in expanded {
            if let Some(caps) = range_re.captures(&base) {
                ...
                let inner = parts[0];
                let step = parts.get(1).and_then(|s| s.parse::<usize>().ok()).unwrap_or(1);

                let range_parts: Vec<&str> = inner.split('-').collect();
                if range_parts.len() != 2 {
                    return Err(Error::Protocol(format!("Invalid range format: [{}]", range_str)));
                }
                ...
                if let (Ok(start), Ok(end)) = (start_str.parse::<i64>(), end_str.parse::<i64>()) {
                    let width = if start_str.starts_with('0') { start_str.len() } else { 0 };
                ...
                // Alpha range
                    ...
                    if start_char.is_ascii_alphabetic() && end_char.is_ascii_alphabetic() {
                        ...
                        while (increment > 0 && current <= end) || (increment < 0 && current >= end) {
                            ...
                            if (increment > 0 && current > 255 - step as u8) || (increment < 0 && current < step as u8) {
                                break;
                            }
                ...
                if replacements.is_empty() {
                    return Err(Error::Protocol(format!("Unsupported or empty range: [{}]", range_str)));
                }
